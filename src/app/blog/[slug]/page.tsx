...
export default async function BlogPostPage({ params }: Props) {
  ...
  const { prev, next } = await getAdjacentPosts(slug, "en");
  ...
  return (
    <main className="container article">
      ...
      <div className="article-intro">
        ...
        {meta.hasOtherLocale ? (
          <p className="article-lang-switch muted">
            Also available in{" "}
            <Link href={`/he/blog/${meta.slug}`}>Hebrew (עברית)</Link>
          </p>
        ) : null}
      </div>
      ...
      <nav className="article-nav" aria-label="Adjacent articles">
        {prev ? (
          <Link href={`/blog/${prev.slug}`} className="article-nav-card">
            ...
            <div>
              <div className="article-nav-card__label">Older</div>
        ...
        {next ? (
          <Link href={`/blog/${next.slug}`} className="article-nav-card">
            <div className="article-nav-card__thumb">
              <Image src={next.image} alt={next.title} fill sizes="80px" className="object-cover" />
            </div>
            <div>
              <div className="article-nav-card__label">Newer</div>
